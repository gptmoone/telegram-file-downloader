// ============================================================
// ربات دانلودر ملی - نسخه نهایی اصلاح‌شده
// ============================================================
...
async function incrementUserStats(env, chatId, fileSizeBytes) {
  ...
  await env.DB.prepare(`
    INSERT INTO user_stats (chat_id, total_files, total_volume_gb)
    VALUES (?, 1, ?)
    ON CONFLICT(chat_id) DO UPDATE SET
...
async function createStarsInvoiceLink(env, chatId, starsAmount) {
  ...
  const body = {
    ...
    description: `دسترسی به امکانات Pro به مدت ۳۰ روز (${starsAmount} Stars)`,
    payload: payload,
    provider_token: "",
    currency: "XTR",
...
async function handleSuccessfulPayment(env, message, TOKEN) {
  ...
  if (payload && payload.startsWith('stars:')) {
    ...
    if (parts.length >= 2 && parts[1] === chatId) {
      await activateProSubscription(env, chatId, `stars_${payment.telegram_payment_charge_id}`, `${stars} ستاره`, TOKEN);
    } else {
...
async function getBranchTotalSize(env, branchName) {
  ...
  const GITHUB_REPO = 'telegram-file-downloader';
  let totalSize = 0;
  const url = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/git/trees/${branchName}?recursive=1`;
  try {
    const res = await fetch(url, {
      headers: {
        'Authorization': `token ${GITHUB_TOKEN}`,
...
async function adminShowChannels(env, chatId, adminSecret, providedSecret, TOKEN) {
  ...
  for (const ch of channels) {
    keyboard.inline_keyboard.push([
      ...
      { text: `❌ حذف ${ch}`, callback_data: `admin_remove_channel:${ch}` }
    ]);
  }
...
// ============================================================
// ارسال پیام همگانی (Broadcast) - اصلاح شده
// ============================================================
async function startBroadcast(env, adminChatId, messageText, TOKEN) {
  const users = await getAllUsers(env);
  const total = users.length;
  if (!total) {
    await sendMessage(adminChatId, "❌ هیچ کاربری در دیتابیس یافت نشد.", ADMIN_KEYBOARD, TOKEN);
    return;
  }
  ...
  broadcastProgress.set(adminChatId, { total, sent: 0, fail: 0, status: 'running', messageId: statusMsgId, startTime });
  ...
  for (let i = 0; i < total; i++) {
    if (broadcastCancelFlag) {
      broadcastProgress.set(adminChatId, { total, sent, fail, status: 'cancelled', messageId: statusMsgId, startTime });
      if (statusMsgId) {
    ...
    try {
      ...
      const sendRes = await Promise.race([
        sendMessage(userChatId, messageText, keyboard, TOKEN),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), BROADCAST_TIMEOUT_MS))
      ]);

      // بررسی اینکه آیا پیام واقعاً ارسال شد
      if (sendRes && sendRes.ok !== undefined) {
        const resData = await sendRes.json().catch(() => null);
      ...
      fail++;
      console.error(`Broadcast error for ${userChatId}:`, err.message);
    }

    // هر ۱۰ کاربر یا آخرین کاربر، وضعیت را آپدیت کن
    if ((i + 1) % 10 === 0 || i === total - 1) {
      const progress = broadcastProgress.get(adminChatId) || {};
      broadcastProgress.set(adminChatId, { ...progress, total, sent, fail, status: 'running' });
      const percent = Math.round((i + 1) / total * 100);
      const elapsedSec = Math.round((Date.now() - startTime) / 1000);
      if (statusMsgId) {
        try {
          await editMessage(adminChatId, statusMsgId,
            `📨 در حال ارسال پیام همگانی...\n\n📊 پیشرفت: ${i + 1} از ${total} (${percent}%)\n✅ موفق: ${sent}\n❌ ناموفق: ${fail}\n⏱️ زمان سپری شده: ${Math.floor(elapsedSec / 60)} دقیقه ${elapsedSec % 60} ثانیه\n🕐 آخرین به‌روزرسانی: ${new Date().toLocaleTimeString('fa-IR')}\n\n⚠️ لغو: /cancel_broadcast`,
            ADMIN_KEYBOARD, TOKEN);
        } catch (e) { console.error('Edit status error:', e); }
      }
    }
  ...
  const elapsedSec = Math.round((Date.now() - startTime) / 1000);
  broadcastProgress.set(adminChatId, { total, sent, fail, status: 'completed', messageId: statusMsgId, startTime });

  if (statusMsgId) {
    try {
      await editMessage(adminChatId, statusMsgId,
        `✅ ارسال پیام همگانی پایان یافت.\n\n📊 نتیجه نهایی:\n👥 کل: ${total}\n✅ موفق: ${sent}\n❌ ناموفق: ${fail}\n📈 درصد موفقیت: ${Math.round(sent / total * 100)}%\n⏱️ زمان کل: ${Math.floor(elapsedSec / 60)} دقیقه ${elapsedSec % 60} ثانیه\n🕐 پایان: ${new Date().toLocaleTimeString('fa-IR')}`,
        ADMIN_KEYBOARD, TOKEN);
    } catch (e) { console.error('Edit final status error:', e); }
  }
}
...
// ============================================================
// پردازش لینک معلق پس از تأیید عضویت
// ============================================================
async function processPendingLink(env, chatId, fileUrl, fileSize, TOKEN) {
  ...
  if (!allowed) {
    await sendMessage(chatId,
      `❌ شما به حداکثر سهمیه روزانه (${limit} فایل) رسیده‌اید. لطفاً فردا دوباره تلاش کنید یا اشتراک Pro تهیه کنید.`,
      MAIN_KEYBOARD, TOKEN);
    return;
  }
  ...
  if (repoSize >= REPO_SIZE_LIMIT_GB) {
    await sendMessage(chatId, `❌ حجم مخزن به حد مجاز (${REPO_SIZE_LIMIT_GB} گیگابایت) رسیده. لطفاً چند ساعت بعد تلاش کنید.`, MAIN_KEYBOARD, TOKEN);
    return;
  } else if (repoSize >= REPO_SIZE_WARNING_GB) {
    await sendMessage(chatId, `⚠️ هشدار: حجم مخزن نزدیک به حد مجاز (${repoSize.toFixed(1)} از ${REPO_SIZE_LIMIT_GB} گیگابایت). پس از دانلود، فایل را حذف کنید.`, MAIN_KEYBOARD, TOKEN);
  }

  const actualFileSize = fileSize || await getFileSize(fileUrl);
  if (actualFileSize && actualFileSize > 2 * 1024 * 1024 * 1024) {
    await sendMessage(chatId, "❌ حجم فایل بیشتر از ۲ گیگابایت است.", MAIN_KEYBOARD, TOKEN);
    return;
  }
...
// ============================================================
// تابع کیبورد اصلی با دکمه مدیریت برای ادمین
// ============================================================
function getMainKeyboardForAdmin(adminChatId, currentChatId) {
  ...
  if (adminChatId && currentChatId === adminChatId) {
    ...
    if (path === '/api/nowpayments-webhook' && request.method === 'POST') {
      try {
        ...
        if (paymentStatus === 'finished') {
          ...
          await activateProSubscription(env, chatId, orderId, `${amountUSD} USD`, TOKEN);
    ...
    if (path === '/api/started' && request.method === 'POST') {
      try {
        ...
        if (user_id) {
          ...
          await sendMessage(chatId, "🔄 پردازش فایل روی گیت‌هاب آغاز شد...", MAIN_KEYBOARD, TOKEN);
        }
        return new Response('OK');
    ...
    if (path === '/api/failed' && request.method === 'POST') {
      try {
        ...
        if (user_id) {
          ...
          await dbDeleteUserState(env, chatId);
          await finishTask(env);
          await sendMessage(chatId, "❌ پردازش فایل با خطا مواجه شد. لطفاً دوباره تلاش کنید.", MAIN_KEYBOARD, TOKEN);
        }
        return new Response('OK');
    ...
    if (path === `/bot${TOKEN}` && request.method === 'POST') {
      try {
        ...
        if (update.message?.text === '/cancel_broadcast' && update.message.chat.id.toString() === ADMIN_CHAT_ID) {
          ...
          await sendMessage(ADMIN_CHAT_ID, "⛔ درخواست لغو ارسال همگانی ثبت شد.", ADMIN_KEYBOARD, TOKEN);
          return new Response('OK');
        }

        // مشاهده وضعیت broadcast
        if (update.message?.text === '/broadcast_status' && update.message.chat.id.toString() === ADMIN_CHAT_ID) {
          ...
          if (!progress) {
            await sendMessage(ADMIN_CHAT_ID, "ℹ️ هیچ ارسال همگانی فعال یا اخیری وجود ندارد.", ADMIN_KEYBOARD, TOKEN);
          } else {
            let statusText = '';
        ...
        // =====================================================
          // دکمه "عضو شدم" - اصلاح شده (بدون async IIFE)
          // =====================================================
          if (data === 'check_membership') {
            try {
              const pending = await getPendingLink(env, chatId);
              if (!pending) {
                await sendMessage(chatId, "❌ لینک معلقی یافت نشد. لطفاً لینک خود را مجدداً ارسال کنید.", MAIN_KEYBOARD, TOKEN);
                return new Response('OK');
              }
          ...
          if (data === 'cancel_input') {
            ...
            await sendMessage(chatId, "❌ عملیات لغو شد. از منوی زیر انتخاب کنید:", welcomeKeyboard, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'discount_pro') {
            const isPro = await isProUser(env, chatId);
            if (isPro) {
              await sendMessage(chatId, "✅ شما قبلاً عضو Pro هستید.", MAIN_KEYBOARD, TOKEN);
              return new Response('OK');
            }
            const discount = await getDiscountSettings(env);
            if (!discount) {
              await sendMessage(chatId, "❌ تخفیف فعلاً فعال نیست. لطفاً از روش معمول استفاده کنید.", MAIN_KEYBOARD, TOKEN);
              return new Response('OK');
            }
            ...
            keyboardRows.push([{ text: "🔙 بازگشت", callback_data: "back_to_main" }]);
            if (keyboardRows.length === 1) {
              await sendMessage(chatId, "❌ خطا در ایجاد لینک پرداخت.", MAIN_KEYBOARD, TOKEN);
            } else {
              const keyboard = { inline_keyboard: keyboardRows };
              await sendMessage(chatId,
                `🎁 تخفیف ویژه!\n\nعضویت Pro فقط با ${discount.starsPrice} ستاره یا ${discount.usdPrice} دلار.\n\nاین پیشنهاد تا ${new Date(discount.expiresAt * 1000).toLocaleString('fa-IR')} معتبر است.`,
                keyboard, TOKEN);
            }
            return new Response('OK');
          }
          ...
          if (data === 'back_to_main') {
            const welcomeKeyboard = getMainKeyboardForAdmin(ADMIN_CHAT_ID, chatId);
            await sendMessage(chatId, "🌀 منوی اصلی", welcomeKeyboard, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'admin_broadcast_status') {
            ...
            if (!progress) {
              ...
              const elapsedSec = Math.round((Date.now() - (progress.startTime || Date.now())) / 1000);
              await sendMessage(ADMIN_CHAT_ID,
                `📊 <b>وضعیت ارسال همگانی</b>\n\n📌 وضعیت: ${statusText}\n👥 کل: ${progress.total}\n✅ موفق: ${progress.sent}\n❌ ناموفق: ${progress.fail}\n📈 درصد: ${Math.round((progress.sent || 0) / (progress.total || 1) * 100)}%\n⏱️ زمان: ${Math.floor(elapsedSec / 60)} دقیقه ${elapsedSec % 60} ثانیه`,
                ADMIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          }
          ...
          if (data === 'admin_promote') {
            ...
            await sendMessage(chatId, "🔹 لطفاً شناسه عددی کاربر (chat id) مورد نظر را وارد کنید:", ADMIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'admin_set_channel') {
            ...
            await sendMessage(chatId, "🔹 لطفاً نام کاربری کانال را وارد کنید (مثال: maramidownload). نیازی به @ نیست.", ADMIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'admin_broadcast') {
            ...
            await sendMessage(chatId,
              "📨 لطفاً متن پیام همگانی را ارسال کنید.\n(برای لغو متن /cancel را بفرستید)\n\n💡 نکته: ارسال پیام در پس‌زمینه انجام می‌شود.\n📊 برای مشاهده وضعیت از دکمه «📊 وضعیت ارسال» استفاده کنید.",
              ADMIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'admin_set_discount') {
            ...
            await sendMessage(chatId, "🎁 لطفاً مدت اعتبار تخفیف را به ساعت وارد کنید (مثال: 24):", ADMIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'admin_clear_discount') {
            ...
            await sendMessage(chatId, "✅ تخفیف فعلی لغو شد.", ADMIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'pro_info') {
            ...
            if (isPro) {
              ...
              if (keyboardRows.length === 0) {
                await sendMessage(chatId, "❌ در حال حاضر روش پرداختی در دسترس نیست. لطفاً بعداً تلاش کنید.", MAIN_KEYBOARD, TOKEN);
              } else {
                keyboardRows.push([{ text: "🔙 بازگشت", callback_data: "back_to_main" }]);
                const proKeyboard = { inline_keyboard: keyboardRows };
                let messageText = `⭐️ <b>عضویت ویژه (Pro)</b>\n\n🎁 <b>مزایا:</b>\n• دانلود فایل شما با اینترنت ملی و صرفه‌جویی در حجم وی‌پی‌ان تا ۱ روز روی سرور می‌ماند (عادی ۱ ساعت)\n• اولویت بالاتر در صف پردازش\n• حداکثر ${DAILY_LIMIT_PRO} فایل و ${DAILY_VOLUME_PRO_BYTES / (1024 * 1024)} مگابایت در روز\n\n💰 <b>هزینه:</b>\n• Telegram Stars: ${starsAmount} ستاره\n• ارز دیجیتال: ${usdAmount} USD (معادل TON)\n\nپس از پرداخت، اشتراک شما بلافاصله فعال می‌شود.`;
                if (discount) {
                  messageText += `\n\n🎁 تخفیف ویژه: ${discount.starsPrice} ستاره (به جای ${STARS_AMOUNT}) / ${discount.usdPrice} USD (به جای ${USD_AMOUNT}) تا ${new Date(discount.expiresAt * 1000).toLocaleString('fa-IR')}`;
                }
          ...
          if (data === 'help') {
            const helpText = `📘 <b>راهنمای ربات</b>\n\n` +
              `این ربات لینک مستقیم فایل را به لینک قابل دانلود با <b>اینترنت ملی و صرفه‌جویی در حجم وی‌پی‌ان</b> تبدیل می‌کند.\n\n` +
              `🔹 <b>نحوه استفاده:</b>\n` +
              ...
              `⭐️ <b>عضویت Pro</b>\n` +
              `• دانلود فایل شما با <b>اینترنت ملی و صرفه‌جویی در حجم وی‌پی‌ان</b> تا <b>۱ روز</b> روی سرور می‌ماند (عادی ۱ ساعت)\n` +
              `• اولویت بالاتر در صف پردازش\n` +
            ...
            await sendMessage(chatId, helpText, MAIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (data === 'stats') {
            try {
              ...
              const proUsersCount = (await env.DB.prepare('SELECT COUNT(*) as count FROM pro_users WHERE expires_at > ?').bind(nowUnix).first()).count || 0;
              const repoSize = await getRepoSize(env);
              ...
              console.error(err);
              await sendMessage(chatId, "⚠️ خطا در دریافت آمار.", MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          }
          ...
          if (data === 'status') {
            try {
              ...
              const state = await dbGetUserState(env, chatId);
              if (!state) {
                await sendMessage(chatId, `📭 هیچ درخواست فعالی ندارید.\n\n${quotaText}${statsText}`, MAIN_KEYBOARD, TOKEN);
                return new Response('OK');
              }
              ...
              if (state.status === 'processing') {
                await sendMessage(chatId, `🔄 وضعیت: در حال پردازش...${progress}\n\n${quotaText}${statsText}`, MAIN_KEYBOARD, TOKEN);
                ...
                await sendMessage(chatId,
                  `⏳ وضعیت: در صف انتظار (شماره صف: ${pos}${isPro ? ' - اولویت Pro' : ''})\n\n${quotaText}${statsText}`,
                  MAIN_KEYBOARD, TOKEN);
              } else if (state.status === 'awaiting_password') {
                await sendMessage(chatId, `🔐 منتظر رمز عبور هستم. لطفاً رمز خود را ارسال کنید.\n\n${quotaText}${statsText}`, MAIN_KEYBOARD, TOKEN);
              } else {
                await sendMessage(chatId, `📭 هیچ درخواست فعالی ندارید.\n\n${quotaText}${statsText}`, MAIN_KEYBOARD, TOKEN);
              }
            } catch (err) {
              console.error(err);
              await sendMessage(chatId, "⚠️ خطا در دریافت وضعیت.", MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          }
          ...
          if (data === 'delete_my_file') {
            try {
              const lastBranch = await dbGetLastBranch(env, chatId);
              if (!lastBranch) {
                await sendMessage(chatId, "❌ هیچ فایل فعالی برای حذف یافت نشد.", MAIN_KEYBOARD, TOKEN);
                return new Response('OK');
              }
              ...
              if (res.ok || res.status === 404) {
                await dbRemoveActiveBranch(env, lastBranch);
                await sendMessage(chatId, "✅ فایل شما از سرور حذف شد.", MAIN_KEYBOARD, TOKEN);
              } else {
                await sendMessage(chatId, "❌ خطا در حذف فایل. لطفاً دوباره تلاش کنید.", MAIN_KEYBOARD, TOKEN);
              }
            } catch (err) {
              console.error(err);
              await sendMessage(chatId, "⚠️ خطا در حذف فایل.", MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          }
          ...
          if (data === 'new_link') {
            ...
            await sendMessage(chatId, "✅ درخواست قبلی لغو شد. اکنون لینک جدید را ارسال کنید.\n(برای دریافت لینک مستقیم از: @filesto_bot)", MAIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
        ...
        if (update.message?.text) {
          ...
          if (adminTempState.has(chatId) && chatId === ADMIN_CHAT_ID) {
            ...
            if (state.step === 'awaiting_broadcast_message') {
              if (text === '/cancel') {
                adminTempState.delete(chatId);
                await sendMessage(chatId, "❌ ارسال همگانی لغو شد.", ADMIN_KEYBOARD, TOKEN);
                return new Response('OK');
              }
              adminTempState.delete(chatId);
              // ارسال در پس‌زمینه بدون await
              startBroadcast(env, chatId, text, TOKEN).catch(e => console.error('Broadcast error:', e));
              await sendMessage(chatId,
                "📨 ارسال پیام همگانی در پس‌زمینه آغاز شد.\n\n📊 برای مشاهده وضعیت، از دکمه «📊 وضعیت ارسال» استفاده کنید.\n⚠️ برای لغو: /cancel_broadcast",
                ADMIN_KEYBOARD, TOKEN);
              return new Response('OK');
            }
            ...
            if (state.step === 'awaiting_discount_duration') {
              ...
              if (isNaN(hours) || hours <= 0) {
                await sendMessage(chatId, "❌ لطفاً یک عدد صحیح مثبت (ساعت) وارد کنید. مثال: 24", ADMIN_KEYBOARD, TOKEN);
                return new Response('OK');
              }
              adminTempState.set(chatId, { step: 'awaiting_discount_stars', hours: hours });
              await sendMessage(chatId, `✅ مدت تخفیف ${hours} ساعت تنظیم شد.\n⭐️ حالا تعداد ستاره تخفیفی را وارد کنید (عدد صحیح):`, ADMIN_KEYBOARD, TOKEN);
              return new Response('OK');
            }
            ...
            if (state.step === 'awaiting_discount_stars') {
              ...
              if (isNaN(starsPrice) || starsPrice <= 0) {
                await sendMessage(chatId, "❌ لطفاً یک عدد صحیح مثبت (ستاره) وارد کنید. مثال: 40", ADMIN_KEYBOARD, TOKEN);
                return new Response('OK');
              }
              adminTempState.set(chatId, { step: 'awaiting_discount_usd', starsPrice: starsPrice, hours: state.hours });
              await sendMessage(chatId, `⭐️ قیمت ستاره تخفیفی: ${starsPrice} تنظیم شد.\n💰 حالا قیمت دلاری تخفیفی را وارد کنید (مثال: 0.7):`, ADMIN_KEYBOARD, TOKEN);
              return new Response('OK');
            }
            ...
            if (state.step === 'awaiting_discount_usd') {
              ...
              if (isNaN(usdPrice) || usdPrice <= 0) {
                await sendMessage(chatId, "❌ لطفاً یک عدد مثبت (دلار) وارد کنید. مثال: 0.7", ADMIN_KEYBOARD, TOKEN);
                return new Response('OK');
              }
              await setDiscount(env, state.starsPrice, usdPrice, state.hours);
              adminTempState.delete(chatId);
              await sendMessage(chatId,
                `✅ تخفیف با موفقیت تنظیم شد.\n💰 قیمت تخفیفی: ${state.starsPrice} ستاره / ${usdPrice} USD\n⏳ اعتبار: ${state.hours} ساعت`,
                ADMIN_KEYBOARD, TOKEN);
              return new Response('OK');
            }
          ...
          if (resetstatsMatch) {
            ...
            if (ADMIN_SECRET && secret === ADMIN_SECRET) {
              ...
              await finishTask(env);
              await sendMessage(chatId, "✅ آمار پردازش‌های فعال و صف بازنشانی شد. صف در حال پردازش است.", ADMIN_KEYBOARD, TOKEN);
            } else {
              await sendMessage(chatId, "❌ دسترسی غیرمجاز.", MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          }
          ...
          if (fixactiveMatch) {
            ...
            if (ADMIN_SECRET && secret === ADMIN_SECRET) {
              ...
              await env.DB.prepare('UPDATE user_state SET status = ? WHERE status = ?').bind('cancelled', 'processing').run();
              await finishTask(env);
              await sendMessage(chatId, `✅ ${processingCount} رکورد پردازش گیر کرده لغو شد. صف در حال پردازش است.`, ADMIN_KEYBOARD, TOKEN);
            } else {
              await sendMessage(chatId, "❌ دسترسی غیرمجاز.", MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          ...
          if (startqueueMatch) {
            ...
            if (ADMIN_SECRET && secret === ADMIN_SECRET) {
              await finishTask(env);
              await sendMessage(chatId, "✅ صف مجدداً راه‌اندازی شد.", ADMIN_KEYBOARD, TOKEN);
            } else {
              await sendMessage(chatId, "❌ دسترسی غیرمجاز.", MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          ...
          if (resetqueueMatch) {
            ...
            if (ADMIN_SECRET && secret === ADMIN_SECRET) {
              await env.DB.prepare('DELETE FROM queue').run();
              await sendMessage(chatId, "✅ صف با موفقیت خالی شد (پردازش‌های جاری دست نخورده).", ADMIN_KEYBOARD, TOKEN);
            } else {
              await sendMessage(chatId, "❌ دسترسی غیرمجاز.", MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          ...
          if (promoteMatch) {
            ...
            await sendMessage(chatId, result, MAIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (text === '/myid') {
            await sendMessage(chatId, `🆔 شناسه چت (Chat ID) شما: <code>${chatId}</code>`, MAIN_KEYBOARD, TOKEN);
            return new Response('OK');
          }
          ...
          if (text === '/start' || text.startsWith('/start ')) {
            await dbDeleteUserState(env, chatId);
            await dbRemoveFromQueue(env, chatId);
            const welcome = `🌀 <b>به ربات دانلودر ملی خوش آمدید</b> 🌀\n\n` +
              `📌 <b>دانلود فایل شما با اینترنت ملی و صرفه‌جویی در حجم وی‌پی‌ان!</b>\n\n` +
              ...
              `• منتظر بمانید تا پردازش شود و لینک دانلود را دریافت کنید.\n\n` +
              `⭐️ <b>عضویت Pro</b>\n` +
              `• دانلود فایل شما با <b>اینترنت ملی و صرفه‌جویی در حجم وی‌پی‌ان</b> تا <b>۱ روز</b> روی سرور می‌ماند (عادی ۱ ساعت)\n` +
              `• اولویت بالاتر در صف پردازش\n` +
              `• حداکثر <b>${DAILY_LIMIT_PRO} فایل و ${DAILY_VOLUME_PRO_BYTES / (1024 * 1024)} مگابایت در روز</b> (عادی ${DAILY_LIMIT_NORMAL} فایل و ${DAILY_VOLUME_NORMAL_BYTES / (1024 * 1024)} مگابایت)\n` +
          ...
          if (userState && userState.status === 'awaiting_password' && userState.requestData) {
            ...
            if (activeCount < MAX_CONCURRENT) {
              ...
              await sendMessage(chatId, "📤 درخواست به گیت‌هاب ارسال شد. منتظر شروع پردازش باشید...", MAIN_KEYBOARD, TOKEN);
            } else {
              ...
              await sendMessage(chatId, `⏳ در صف قرار گرفتید. شماره صف: ${pos}${isPro ? ' (اولویت Pro)' : ''}`, MAIN_KEYBOARD, TOKEN);
            }
            return new Response('OK');
          }

          // ========== در غیر این صورت ==========
          await sendMessage(chatId, "❌ لینک معتبر نیست (باید با http:// یا https:// شروع شود).", MAIN_KEYBOARD, TOKEN);
          return new Response('OK');
        }
