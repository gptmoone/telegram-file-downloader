// ============================================================
// ربات دانلودر ملی نسخه نهایی (بدون فشرده‌سازی، رفع مشکل ادمین)
// ============================================================
...
const TTL_NORMAL = 3600;      // 1 ساعت برای کاربر عادی
const TTL_PRO = 86400;        // 1 روز برای کاربر Pro
const DAILY_LIMIT_NORMAL = 2;   // حداکثر تعداد فایل عادی در روز
const DAILY_LIMIT_PRO = 6;      // حداکثر تعداد فایل Pro در روز
...
async function getRepoSize(env) {
  ...
  try {
    ...
    console.error('getRepoSize error:', e);
  }
  return 0;
}
...
async function startBroadcast(env, adminChatId, messageText, TOKEN) {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    try {
      ...
      await sendMessage(userId, messageText, keyboard, TOKEN);
      successCount++;
    } catch (err) {
      failCount++;
      console.error(`Broadcast failed for ${userId}:`, err);
    }
  ...
  async fetch(request, env) {
    ...
    if (path === '/api/started' && request.method === 'POST') {
      try {
        ...
        console.error('/api/started error:', err);
        return new Response('OK');
      }
    }
    ...
    if (path === `/bot${TOKEN}` && request.method === 'POST') {
      try {
        ...
        if (update.callback_query) {
          ...
          if (data === 'check_membership') {
            (async () => {
              ...
              if (!pending) {
                await sendSimple(chatId, "❌ لینک معلقی یافت نشد.", TOKEN);
                return;
              }
          ...
          if (data === 'discount_pro') {
            ...
            const discount = await getDiscountSettings(env);
            if (!discount) {
              await sendSimple(chatId, "❌ تخفیف فعلاً فعال نیست.", TOKEN);
              return new Response('OK');
            }
          ...
          if (data === 'pro_info') {
            ...
            if (isPro) {
              ...
              if (keyboardRows.length === 0) {
                await sendSimple(chatId, "❌ در حال حاضر روش پرداختی در دسترس نیست.", TOKEN);
              } else {
          ...
          if (data === 'delete_my_file') {
            ...
            if (res.ok || res.status === 404) {
              ...
              await sendSimple(chatId, "❌ خطا در حذف فایل.", TOKEN);
            }
            return new Response('OK');
          }
          if (data === 'new_link') {
        ...
        // ---------- پیام متنی (با اصلاح ترتیب اولویت) ----------
        if (update.message?.text) {
          ...
          if (adminTempState.has(chatId) && ADMIN_CHAT_ID && chatId === ADMIN_CHAT_ID) {
            ...
            // اگر هیچکدام، پاک کن و ادامه بده (نباید پیش بیاید)
            adminTempState.delete(chatId);
          }

          // ✅ اولویت 2: دستورات ادمین متنی (که قبلاً در بخش بالا هم بودند، اما برای امنیت بیشتر)
          ...
          if (text.startsWith('/resetqueue')) {
            ...
            if (ADMIN_SECRET && secret === ADMIN_SECRET) {
              await env.DB.prepare('DELETE FROM queue').run();
              await sendSimple(chatId, "✅ صف با موفقیت خالی شد.", TOKEN);
            } else {
              await sendSimple(chatId, "❌ دسترسی غیرمجاز.", TOKEN);
            }
            return new Response('OK');
